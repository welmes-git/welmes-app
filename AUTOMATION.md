...
- **세션**: 슈퍼딜리버리는 Cloudflare Turnstile 때문에 세션 만료 시 headful 로그인 창이 뜬다.
  스케줄 실행은 맥 사용 중인 시간대(10:00)로 설정 — 로그인 창에 응답 가능.
  세션 파일 `scripts/.sd-session.json`은 git 제외.
- **속도**: 상품당 약 2초 (예의 지연 포함). 300개 ≒ 10분.
